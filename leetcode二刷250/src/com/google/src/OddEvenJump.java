 ...
 * 6th, ...) jumps in the series are called even-numbered jumps. Note that the jumps are
 * numbered, not the indices.
 *
 * You may jump forward from index i to index j (with i < j) in the following way:
 *
 * During odd-numbered jumps (i.e., jumps 1, 3, 5, ...), you jump to the index j such that arr[i]
 * <= arr[j] and arr[j] is the smallest possible value. If there are multiple such indices j, you
 * can only jump to the smallest such index j.
 * During even-numbered jumps (i.e., jumps 2, 4, 6, ...), you jump to the index j such that
 * arr[i] >= arr[j] and arr[j] is the largest possible value. If there are multiple such indices
 * j, you can only jump to the smallest such index j.
 ...
 * Output: 2
 * Explanation:
 * From starting index i = 0, we can make our 1st jump to i = 2 (since arr[2] is the smallest
 * among arr[1], arr[2], arr[3], arr[4] that is greater or equal to arr[0]), then we cannot jump
 * any more.
 * From starting index i = 1 and i = 2, we can make our 1st jump to i = 3, then we cannot jump
 * any more.
 ...
 * During our 1st jump (odd-numbered), we first jump to i = 1 because arr[1] is the smallest
 * value in [arr[1], arr[2], arr[3], arr[4]] that is greater than or equal to arr[0].
 ...
 * largest value, but 2 is a smaller index, so we can only jump to i = 2 and not i = 3
 * During our 3rd jump (odd-numbered), we jump from i = 2 to i = 3 because arr[3] is the smallest
 * value in [arr[3], arr[4]] that is greater than or equal to arr[2].
 ...
 * In total, there are 3 different starting indices i = 1, i = 3, and i = 4, where we can reach
 * the end with some
 * number of jumps.
 * Example 3:
 *
